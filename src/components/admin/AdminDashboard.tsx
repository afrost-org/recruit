...
const AdminDashboard = ({ password }: AdminDashboardProps) => {
  ...
  const filtered = useMemo(() => {
    return applications.filter((app) => {
      ...
      if (search) {
        ...
        return matchesAnswers || matchesEmail || matchesTitle;
      }
      return true;
    });
  ...
  const grouped = useMemo(() => {
    const groups: Record<string, { title: string; apps: ApplicationRecord[] }> = {};
      ...
      {Object.keys(grouped).length === 0 ? (
        ...
        <div className="space-y-4">
          {Object.entries(grouped).map(([jobId, group]) => {
            const newCount = group.apps.filter((a) => a.status === "new").length;
            return (
